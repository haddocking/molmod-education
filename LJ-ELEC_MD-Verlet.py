...
# Insert below the lines defining the new x and y positions based on the
# old ones + forces + mass + dtstep=h
#>>>>>
...
# Calculate the velocities from old and current coordinates
...
def Go(*args):
  global Atom_Coord,Radius,Mass,BoxDim,Epsilon,Rmin,CutOffSquare,Iterations,Ene,Accepted,Old_Atom_Coord
  ...
  if Iterations==0:
       ...
       Atom_Coord=Step1(Atom_Coord,Velocity,Force,timestep,Mass)
       Ene,EneLJ,EneCoul = Calc_Ene2(Atom_Coord,Epsilon,Rmin,Dielec,CutOffSquare,BoxDim)
  ...
  if nout == 20:
    nout = 0
    print("step: %d Time: %8.3f Etot: %6.1f Ekin: %6.1f Epot: %6.1f Elj: %6.1f Ecoul: %6.1f Temp: %6.1f" % (Iterations,float(Iterations)*timestep,Ene+Kin,Kin,Ene,EneLJ,EneCoul,temperature))
...
def set_tstep(event):
 global Atom_Coord,ATOM,Color,canevas,Radius,Iterations,timestep,Velocity,cstboltz
...
### update energy ###
def update_ene():
  global Atom_Coord,Radius,BoxDim,Epsilon,Rmin,CutOff,CutOffSquare,Iterations,Ene
