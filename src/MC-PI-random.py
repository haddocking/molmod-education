...
def Go(*args):
    ...
    if dist2(x, y, Center) <= R2:
        ncircle = ncircle + 1.0
    # update the MC estimate of pi every step (not only on hits)
    pi = 4 * ncircle / counter
    # keep track of the change per iteration and the error in the computed value of pi
    frac_change = 100.0 * abs(pi - pi_old) / pi if pi > 0 else 0.0
    error = 100.0 * abs(tpi - pi) / tpi

    # and now calculate it using bailey's sum:
    # Pi = SUMk=0 to infinity 16-k  [ 4/(8k+1) - 2/(8k+4) - 1/(8k+5) - 1/(8k+6) ].

    pi_bailey = pi_bailey + 16 ** (-counter) * (
        4.0 / (8.0 * counter + 1.0)
        - 2.0 / (8 * counter + 4.0)
        - 1.0 / (8.0 * counter + 5.0)
        - 1.0 / (8.0 * counter + 6.0)
    ...
    x2 = x - Radius
    y2 = y - Radius
    xr = canevas.create_oval(x1, y1, x2, y2, fill="#FFFFFF")
    if lcounter == 50.0:
        print(
            "Step ",
            counter,
        ...
        if counter > 50.0:
            ...
            sttext1.destroy()
        lcounter = 0
        mynewtext = "step: %10d PI= %11.8f %%error %8.5f %%change %8.5f" % (
            counter,
            pi,
            error,
            frac_change,
        ...
        mynewtext = "Bailey's sum PI= %11.8f %%error %8.5f %%change %8.5f" % (
            pi_bailey,
            werror,
            wfrac_change,
        )
