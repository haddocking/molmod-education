...
normFmin = 0.0001  # minimum force norm to step EM
Seed = 100  # random number seed
numsteep = 0  # number of initial steepest descent EM steps
...
def Steepest_descent(atom_coord, drstep, force):
    global sdir, sdirprev, forceprev
    # the first step
    list = []
...
def Conjugate_gradient(atom_coord, drstep, force):
    global sdir, sdirprev, forceprev
    # the first step
    list = []
    ...
    # - avoid dividing by zero
    #
    # 1) First calculate the norm of the total force vectors (current and previous step)
    #
    normf2 = 0.0
    normf2prev = 0.0
    ...
    # Calculate gamma over total force
    if normf2prev > 0:
...
def Go(*args):
    ...
    global forceprev, sdir, sdirprev
    hwtextene1.destroy()
    hwtextene2.destroy()
    sttext0.destroy()
    ...
    if Iterations == 0:
        ...
        Ene_prev = Ene
        outtext = "Iteration: %8d Epot: %6.1f Elj: %6.1f Ecoul: %6.1f" % (
            Iterations,
            Ene,
            EneLJ,
            EneCoul,
    ...
    if Iterations == 0 or Iterations < numsteep:
        Atom_Coord, normF = Steepest_descent(Atom_Coord, drstep, Force)
    else:
        Atom_Coord, normF = Conjugate_gradient(Atom_Coord, drstep, Force)
    Ene, EneLJ, EneCoul = Calc_Ene2(
        Atom_Coord, Epsilon, Rmin, Dielec, CutOffSquare, BoxDim
    )
    ...
    if nout == 20:
        nout = 0
        outtext = (
            "Iteration: %8d Epot: %6.1f Elj: %6.1f Ecoul: %6.1f deltaE: %10.6f <normF>: %8.6f dr: %8.6f"
            % (Iterations, Ene, EneLJ, EneCoul, Ene_diff, normF, drstep)
        )
        print(outtext)
...
def setupall(Atom_Coord, ATOM, Color, Repack=1):
    ...
    global force, forceprev, sdir, sdirprev
    ...
    for i in range(len(Atom_Coord)):
        ...
        forceprev.append([0.0, 0.0])
        sdir.append([0.0, 0.0])
        sdirprev.append([0.0, 0.0])
...
### set the number of initial steepest descent EM ###
def set_numsteep(event):
    global Atom_Coord, ATOM, Color, canevas, Radius, Iterations
    global r, nAtoms, drstep, drinit, beta, numsteep
    numsteep = float(nsteep.get())
    canevas.destroy()
    canevas = Canvas(root, width=BoxDim[0], height=BoxDim[1], bg="#ccddff")
    canevas.bind("<Button-1>", Go)
    Atom_Coord, ATOM, Color = setupall(Atom_Coord, ATOM, Color, 0)
...
mynewtext3 = "numsteepEM = "
hwtext3 = Label(low, text=mynewtext3)
hwtext3.pack(side="left")
nsteep.set(numsteep)
nsteep_entry = Entry(low, width=6, textvariable=nsteep)
nsteep_entry.pack(side="left")
nsteep_entry.bind("<Return>", set_numsteep)
